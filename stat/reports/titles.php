...
if ($by == "hits") {
    ...
    $selcn = "DISTINCT stat_log.id_session, stat_log.titlepage";
}elseif ($by == "hosts") {
...
$r = mysql_query("SELECT titlepage, COUNT(".$selcn.")
                  FROM stat_log
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
	if (strlen(mysql_result($r, $i, 0)) > 0) $title[] = mysql_result($r, $i, 0);
    else $title[] = "<u><b>!!! ПУСТОЙ ЗАГОЛОВОК !!!</b></u>";
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
    ...
    $html .= "<tr class=\"".$class."\">\n";
   	$html .= '<td align="left" width="550"><font class="">'.$title[$i].'</font>';
