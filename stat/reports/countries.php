...
if ($by == "hits") {
    ...
    $selcn = "DISTINCT stat_log.id_session, stat_log.country";
}elseif ($by == "hosts") {
...
$r = mysql_query("SELECT stat_countries.name AS country, COUNT(".$selcn."), stat_countries.domain
                  FROM stat_log
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
    if (mysql_result($r, $i, 0)=='') $country[] = 'Не определено';
    else $country[] = mysql_result($r, $i, 0);
    $dc[] = mysql_result($r, $i, 2);
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
	...
	$html .= "<tr class=\"".$class."\">\n";
   	$html .= "\t<td align=left width=550><img src='img/countries/".$dc[$i].".gif' width=20 height=12>&nbsp;".$country[$i];
