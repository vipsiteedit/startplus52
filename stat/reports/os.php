...
if ($by == "hits") {
    ...
    $selcn = "DISTINCT stat_log.id_session, stat_log.os";
}elseif ($by == "hosts") {
...
$r = mysql_query("SELECT stat_datas.name AS os, COUNT(".$selcn.")
                  FROM stat_log
                  INNER JOIN stat_datas ON stat_datas.id = stat_log.os
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
	...
	$html .= "<tr class=\"".$class."\">\n";
   	$html .= "\t<td align=left width=550>".$os[$i];
