...
if ($by == "hits") {
    ...
    $selcn = "DISTINCT stat_log.id_session, stat_log.screensize";
}elseif ($by == "hosts") {
	...
	$ADMENU .= "Для пользователей<br>";
	$ADMENU .= "<a href='index.php?".se_stat_sqs("by", "views")."'>Для просмотров</a><br>";
    $selcn = "DISTINCT stat_log.id_session, stat_users.screensize";
}

$r = mysql_query("SELECT stat_users.screensize, COUNT(".$selcn.")
                  FROM stat_log
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
    if (mysql_result($r, $i, 0)=='') $screensize[] = "Не определено";
    else $screensize[] = mysql_result($r, $i, 0);
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
	...
	$html .= "<tr class=\"".$class."\">\n";
   	$html .= "\t<td align=left width=550>".$screensize[$i];
