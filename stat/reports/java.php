...
if ($by == "hits") {
    ...
    $selcn = "DISTINCT stat_log.id_session, stat_users.java";
}elseif ($by == "hosts") {
...
$r = mysql_query("SELECT stat_users.java, COUNT(".$selcn.")
                  FROM stat_log
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
    if (mysql_result($r, $i, 0)=='Y') $java[] = "Да";
    else $java[] = "Нет";
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
	...
	$html .= "<tr class=\"".$class."\">\n";
   	$html .= "\t<td align=left width=550>".$java[$i];
