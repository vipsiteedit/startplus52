...
if ($by == "hits") {
    ...
    $selcn = "DISTINCT stat_log.id_session, stat_users.cookies";
}elseif ($by == "hosts") {
...
$r = mysql_query("SELECT stat_users.cookies AS `cookies`, COUNT(".$selcn.") AS `cn`
                  FROM stat_log
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
    if (mysql_result($r, $i, 'cookies')=='Y') $cookies[] = "Да";
    else $cookies[] = "Нет";
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
	...
	$html .= "<tr class=\"".$class."\">\n";
   	$html .= "\t<td align=left width=550>".$cookies[$i];
