...
$r = mysql_query("SELECT stat_datas.name AS ref_t, COUNT(".$selcn.")
                  FROM stat_log
                  INNER JOIN stat_datas ON stat_datas.id = stat_log.ref_rating
                  WHERE ((stat_log.date >= '".$begdate."')AND(stat_log.time >= '".$begtime."')) AND
                        ((stat_log.date <= '".$enddate."')AND(stat_log.time <= '".$endtime."')) AND
                        (stat_log.ref_rating>0)
                  GROUP BY 1
                  ORDER BY 2 desc
                  LIMIT ".$limitpage.";");
$rs = mysql_query("SELECT COUNT(".$selcn.")
                  FROM stat_log
                  INNER JOIN stat_datas ON stat_datas.id = stat_log.ref_rating
                  WHERE ((stat_log.date >= '".$begdate."')AND(stat_log.time >= '".$begtime."')) AND
                        ((stat_log.date <= '".$enddate."')AND(stat_log.time <= '".$endtime."')) AND
