...
if ($by == "hits") {
    ...
    $selcn = "DISTINCT stat_log.id_session, stat_log.ref_catalog";
}elseif ($by == "hosts") {
...
$rd = mysql_query("SELECT name FROM stat_datas WHERE type=0");
while ($row = mysql_fetch_array($rd, MYSQL_BOTH)) $notindomains[] = $row['name'];
if (!empty($notindomains)) $notind = "'".join("','", $notindomains)."'";
else $notind = "'_'";

$r = mysql_query("SELECT stat_datas.name AS `ref_с`, COUNT(".$selcn.")
                  FROM stat_log
                  INNER JOIN stat_datas ON stat_datas.id = stat_log.ref_catalog
                  WHERE ((stat_log.date >= '".$begdate."')AND(stat_log.time >= '".$begtime."')) AND
                        ((stat_log.date <= '".$enddate."')AND(stat_log.time <= '".$endtime."')) AND
                        (stat_log.ref_catalog>0)
                  GROUP BY `ref_с`
                  ORDER BY 2 desc
                  LIMIT ".$limitpage.";");
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
	...
	$html .= "<tr class=\"".$class."\">\n";
   	$html .= "\t<td align=left width=550><a href='http://".$move_c[$i]."' target='_blank' title='перейти на сайт'>".$move_c[$i]."</a>";
