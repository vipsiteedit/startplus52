...
$rdt = mysql_query("SELECT MAX(id) AS `id`
                  FROM stat_log
...
// сокращенный путь
$r = mysql_query("SELECT SQL_CALC_FOUND_ROWS COUNT(page) AS `cn`, page AS `page`, id_user AS `idus`, id_session AS `ids`,
                         domain, titlepage
                  FROM stat_log
...
// полный путь
$r = mysql_query("SELECT SQL_CALC_FOUND_ROWS COUNT(request_uri) AS `cn`, request_uri AS `page`, id_user AS `idus`, id_session AS `ids`,
                         domain, titlepage
                  FROM stat_log
                  WHERE (stat_log.id IN (".$indt."))
...
for ($i=0; $i < mysql_num_rows($r); $i++) {
	$outputpage[] = mysql_result($r, $i, 'page');
...
if (!empty($outputpage[$i])) {
	...
	$html .= "<tr class=".$class.">\n";
   	$html .= "\t<td align=left width=550>
                  <a href='http://".$domain[$i].$outputpage[$i]."' target='_blank' title='перейти на сайт'>".$outputpage[$i]."</a>
