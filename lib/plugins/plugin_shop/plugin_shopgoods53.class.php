...
class plugin_shopgoods53 {
    ...
    public function getGoodsFeatures($id_goods) {
        ...
        if (!empty($id_goods)) {
            ...
            $shop_feature->select("DISTINCT sf.id,
                ...
                GROUP_CONCAT(CASE    
                    WHEN (sf.type = 'list' OR sf.type = 'colorlist') THEN (SELECT CONCAT_WS('', sfvl.value, '##', sfvl.color, '##', sfvl.image, '##', sfvl.sort) FROM shop_feature_value_list sfvl WHERE sfvl.id = smf.id_value)
				...
				foreach($featurelist as $val) {
					...
					if ($val['type'] == 'colorlist' || $val['type'] == 'list') {
						...
						foreach($list_values as $value){
							list($val['val'], $color, $image, $sort) = explode('##', $value);
							$val['value'][(int)$sort] = $val['val'];
						}
						ksort($val['value']);
    ...
    **/
    public function getGoods($options = array(), $goods_name = '', $on_count = false) {
		...
		if (empty($goods_name)) {
			...
			if (isRequest('q', 1)) {
				...
				foreach($words as $word){
					$price->andWhere('(sp.name LIKE "%?%" OR sp.article LIKE "%?%" OR sp.note LIKE "%?%")', $word);
				}
			}
		...
		elseif ($option['not_available'])
			$price->andWhere('(sp.presence_count=0 AND (SELECT 1 FROM shop_modifications WHERE id_price=sp.id AND (`count` IS NULL OR `count`<>0) LIMIT 1) IS NULL)');
