	...
	function DirStructure($type, $innerDirs='first', $currentDir='', $level=0) {
		...
		foreach ($innerDirs as $v) {
			...
			if(isset($v['childs'])) {
				...
				//$ret .= '|'.$v['name'].'|';
				$ret .= '<div class="'.$folderClass.' '.$folderAct.'" path="'.$v['path'].'" title="'.$files.'" pathtype="'.$type.'">'.str_replace($this->dir[$type],'',$v['name']).($v['count']>0?' ('.$v['count'].')':'').'</div><div class="folderOpenSection" '.$folderOpen.'>';
				...
				$ret .= '<div class="folderClosed '.$folderAct.'" path="'.$v['path'].'" title="'.$files.'" pathtype="'.$type.'">'.str_replace($this->dir[$type],'',$v['name']).($v['count']>0?' ('.$v['count'].')':'').'</div>'; 
			}
		}
	...
	function DelFile($pathtype, $path, $md5, $filename, $callShowDir=false) {
		...
		if(is_file($path.'/'.$filename)) {
			if(unlink($path.'/'.$filename)) {
				$prev_image = explode('.', $filename);
				$prev_image = $prev_image[0].'_prev.'.$prev_image[1];
				if (file_exists($path.'/'.$prev_image)) unlink($path.'/'.$prev_image);
				if($callShowDir) {
