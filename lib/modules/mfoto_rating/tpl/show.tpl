        ...
        <div class="content cont_photo_rating photoDetailed record-item"<?php echo $__data->editItemRecord($section->id, $record->id) ?> id="view">
            ...
            <?php if(!empty($record->image)): ?>
                <div class="objimage record-image" id="objimage">
                    <img class="fullImage objectImage" alt="<?php echo $record->image_alt ?>" src="<?php echo $record->image ?>" border="0">
                </div>
            <?php endif; ?>
...
<?php endif; ?>
            <input class="buttonSend" onclick="document.location = '<?php echo seMultiDir()."/".$_page."/" ?>'" type="button" value="<?php echo $section->language->lang004 ?>">
        </div> 
