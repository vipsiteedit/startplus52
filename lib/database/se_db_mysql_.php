...
function se_db_connect($link = 'db_link')
{
 ...
 // mysql_query("set collation_connection='cp1251_general_ci'", $$link);
  ...
  //mysql_query("set collation_connection='UTH8'", $$link);
...
function se_db_perform_restrict($table, $data){
	...
	foreach($correcttable as $line){
	    if (empty($line)) continue;
	    $line = explode("|", $line);
	    if (!empty($line[1]) && trim($line[1]) == $table){
	        $id = $data[trim($line[2])];
		if ($id == '' || $id == 'null'){
		    return true;
		}
...
{
  if (strpos($table, 'stat_')!==false || $table == 'se_search') return;
  if (@mysql_num_rows(mysql_query("SHOW COLUMNS FROM `log_synchro`")) == 0)
  {
    mysql_query("CREATE TABLE `log_synchro` (
  ...
  if (@mysql_num_rows(mysql_query("SELECT FIELD FROM `log_synchro` WHERE FIELD  = `fields`")) == 0)
  	mysql_query("ALTER TABLE `log_synchro` ADD `fields` varchar(255);");
  
  
  $deltime = time() - 86400 * 10;
  if ($typech == 'u' || $typech == 'd') {
  	mysql_query("DELETE FROM `log_synchro` WHERE `timesynchro`<'$deltime'");
  }
  if ($typech == 'u'){
    mysql_query("DELETE FROM `log_synchro` WHERE `typech` = 'u' AND `tabl`='{$table}' AND `id_rec`='{$id_rec}';");
  }
  else
    if ($typech == 'd'){
      mysql_query("DELETE FROM `log_synchro` WHERE `typech` IN ('i','u') and `id_rec`='{$id_rec}'");
    }
