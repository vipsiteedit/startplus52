...
var ImagesDialog = {
	...
	insert : function(text) {
		window.opener.addImage(text);//getElementById('recimage').value = text;
		//window.close();
	}
};
		...
		$( "#dialog-modal-image" ).dialog({
			...
			beforeClose: function(event, ui) {
			   // alert(ImagesSrc);
				$('.imageBlock0').die();
				//alert('!!!');
				//$( "#dialog-modal-image" ).dialog( "destroy" );
				//swfu
...
$(function(){
	...
	$.ajax({
		type: "POST",
		url: "/lib/imanager/connector.php",
	...
	$('#toBeginBtn').mouseover(function(){
		$(this).children(0).attr('src','/lib/imanager/img/backActive.gif');
	});
	$('#toBeginBtn').mouseout(function(){
		$(this).children(0).attr('src','/lib/imanager/img/backEnabled.gif');
	});
	...
	$('#fileOpen').MultiFile({ 
		STRING: { 
			remove:'<img src="/lib/imanager/img/cross_small.png" width="16" height="16" alt="Убрать" />',
...
//SWFUpload.onload = function () {
	var settings = {
		...
		upload_url: "/lib/imanager/connector.php",
		post_params: {
			//"PHPSESSID" : "NONE",
