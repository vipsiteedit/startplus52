...
class Image_Toolbox {
	...
	function Image_Toolbox() {
		...
		if ($gd_info['JPG Support'] || $gd_info['JPEG Support']) {
		  // было JPEG Support (вызывалась фатальна€ ошибка)
	...
	function _gd_info() {
		...
		foreach(explode("\n", $info) as $line) {
			...
			if(strpos($line, "JPEG Support") !== false)
				$array["JEPG Support"] = trim(str_replace("JPEG Support", "", strip_tags($line)));
	...
	function _addImage($argc, $args) {
		if (($argc == 2 || $argc == 3) && is_int($args[0]) && is_int($args[1]) && (is_string($args[2]) || !isset($args[2]))) {
			...
			//bild aus datei laden. width und height original grФsse
