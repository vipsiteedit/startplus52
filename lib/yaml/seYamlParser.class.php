...
class seYamlParser
{
    ...
    while ($this->moveToNextLine())
    {
      ...
      if (preg_match('#^\-(\s+(?P<value>.+?))?\s*$#', $this->currentLine, $values))
      {
        ...
        else
        {
          ...
          {
            $data[] = array($matches[1] => seYamlInline::load($matches[2]));
          }
          else
          {
      ...
      {
        $key = seYamlInline::parseScalar($values['key']);
      ...
      else
      {
        ...
        if (1 == count(explode("\n", rtrim($this->value, "\n"))))
        {
          $value = seYamlInline::load($this->lines[0]);
          if (is_array($value))
          {
            $first = reset($value);
            if ('*' === substr($first, 0, 1))
  ...
  protected function parseValue($value)
  {
    ...
    {
      return seYamlInline::load($value);
    }
  }
