...
private $unique;
  ...
  public function __construct($seData, $lang = 'ru')
  {
	...
	$this->lang = $lang;
	// Загружаем переводы
	$ymlfile = SE_CORE . 'editor/i18n/' . $this->lang .'.yml';
    if (file_exists( $ymlfile ))
    { 
        $ymlres = seYAML::Load( $ymlfile );
        foreach($ymlres as $classname=>$this->langmess);
    }
	$this->title_icon_edsection = $this->getTextLanguage('edit_section', 'icon_');
	$this->title_icon_addrecord = $this->getTextLanguage('add_record', 'icon_');
	$this->title_icon_edrecord = $this->getTextLanguage('edit_record', 'icon_');
	$this->title_icon_addsection = $this->getTextLanguage('add_section', 'icon_');
...
// ###############################  EDITORS METHODS ###############################
  ...
  if (!$this->editorAccess()) return;
	...
	if (isset($_GET['on_ajax_execute']))
	{
		...
		if (preg_match("/(edit|add)page(?:_)?([\w]*)/", $_POST['name'], $m)) {
			//var_dump($m);
			$oper = $m[1];
			$tpl = 'content';
	    ...
	    if (preg_match("/editrecord([\w]+)/", $_POST['name'], $recm)){
			list($id_section, $id_object) = explode('_', $_POST['value']);
			$section = $this->data->getSection($id_section);
			$record = $this->data->getObject($section, $id_object);
	    ...
	    if (preg_match("/showrecord([\w]+)/", $_POST['name'], $m)){
			list($id_section, $id_object) = explode('_', $_POST['value']);
			$section = $this->data->getSection($id_section);
			$record = $this->data->getObject($section, $id_object);
			if ($m[1] == 'image'){
	    ...
	    if (preg_match("/editsection([\w]+)/u", $_POST['name'], $recm)){
			$id_section = intval($_POST['value']);
			$section = $this->data->getSection($id_section);
			include dirname(__FILE__).'/../editor/part_editor_'.strtolower($recm[1]).'.tpl';
		}

	    if (preg_match("/showsection([\w]+)/", $_POST['name'], $recm)){
			$id_section = intval($_POST['value']);
			$section = $this->data->getSection($id_section);
			if ($recm[1] == 'image'){
  ...
  private function setSection($id_section, $type = 'mtext'){
		...
		if (isset($_POST['recid'])){
			...
			if (isset($_POST['recordimage'])) {
				$recimage = stripslashes(getRequest('recordimage', 5));
				if ((strval($record->image)!=$recimage || $resize_width) && file_exists(SE_ROOT.SE_DIR.$recimage)) {
...
public function getBtnGroup($buttons, $add) {
  ...
  public function groupWrapper($content_id, $text){
	if (!$this->editorAccess() || $_SESSION['siteediteditor'] || ($this->data->req->object || $this->data->req->sub)) {
		return $text;
	}
  ...
  private function getContentEditLink($section){
    ...
    $img = ($id_sect < 100000) ? "/admin/assets/icons/edit_content.png" : "/admin/assets/icons/edit_gcontent.gif";
  ...
  public function getTextLanguage($word, $key='ed_'){
    if (!empty($this->langmess[$key.$word]))
		return $this->langmess[$key.$word];
    else return $word;
  }
}
?>