...
function apply_img(file,external){
    ...
    path = path.replace('\\', '/');
    var base_url = $('#base_url').val();
	var fm_path = $('#fm_path').val();
	console.log(base_url,fm_path);
    if (external!="") {
	    var file_path = base_url+path+file;
	    
	    if (fm_path.length && file_path.indexOf(fm_path)===0) {
	    	console.log(fm_path.length,file_path.substr(0,fm_path.length));
	    	file_path = file_path.substr(fm_path.length);
	    }
		var target = $('#'+external, window_parent.document);
...
function close_window() {
   if ($('#popup').val()==1) window.close();
   	...
   	if (typeof parent.modal_id !== 'undefined') {
   		parent.jQuery('#'+parent.modal_id).modal('hide');
   	}
